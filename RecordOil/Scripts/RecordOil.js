...
function ZayavkaEditSave() {
    ...
    var data = {
        'ID': $('#ID').val(),
        'NumberDogE': $('#NumberZE').val(),
        'DateDogME': $('#DateZE').val(),
        'OrganizDogE': $('#OrganizDogE').val(),
        'PrimDogE': $('#PrimDogE').val(),
        'UserZayE': $('#UserZayE').val(),
        'DateZayE': $('#DateZayE').val(),
        'DatePlanZE': $('#DatePlanZE').val(),
        'TabZayE': ddd
    };
...
//-----Редактирование заявок в ОННиУК--------------------------------------------------
//Редактирование заявки//

function ZayavkaTotalsEdit(ID) {
    $.ajax({
        url: "/Home/ZayavkaTotalsEdit/" + ID,
        type: "GET",
        contentType: "application/json;charset=UTF-8",
        data: JSON.stringify(),
        dataType: "html",
...
function ZayavkaTotalsEditSave() {
    ...
    $.ajax({
        url: "/Home/ZayavkaTotalsEditSave",
        type: "POST",
        contentType: "application/json;charset=UTF-8",
        data: JSON.stringify(data),
        dataType: "html",
...
function PrintZayList() {
    ...
    $.ajax({
        ...
        success: function Rep(result) {
            $('#ServicesModalContent').html(result);
            $('#ServicesModal').modal('show');
            stringhref += "IDF=" + $('#IdOrgan').val() + "&" + "Soglas=" + $('#Soglas').val() + "&" + "SZ=" + $('#SZ').val() + "&" + "PoZ=" + $('#PoZ').val() + "&" + "SZM=" + $('#SZM').val() + "&" + "PoZM=" + $('#PoZM').val()
            
            /*stringhref += "SZ=" + $('#SZ').val() + "&" + "PoZ=" + $('#PoZ').val() + "&" + "SZM=" + $('#SZM').val() + "&" + "PoZM=" + $('#PoZM').val()*/
            window.location = stringhref;
            setTimeout(function () {
                $("#ServicesModal").modal('hide');
            }, 3000);
...
//----Новый список заявок для регистрации------------------------------------------------//

//----Печать списка заявок----------------------------//
function PrintZayListRegistration() {
    ...
    var stringhref = "/Home/PrintZayListRegistration?";
...
//----Печать списка заявок в раздере ОННИУК----------------------------//
function PrintZayListRegistrationTot() {
    ...
    var stringhref = "/Home/PrintZayListRegistrationTot?";
...
function ZayTotalSave() {
    ...
    for (var it = 1; it <= tList.rows.length; it++) {
        var d =
        {
            ...
            'NalSkladTotal': $('#NalSkladITOG_' + Number(it)).val(),
            'PlanVITOG': $('#PlanVITOG_' + Number(it)).val(),
        };

        var MDT =
        {
            'IdNeft': $('#IDneftTotalITOG_' + Number(it)).val(),
            'Massa': $('#PlanVITO_' + Number(it)).val()           
        }
        ...
        //    dZay.push(d);
        //}
        if ($('#Soglas_' + Number(it)).val() != "Не согласована!")
        {
         dZay.push(d);
        }
        

        if ($('#IDneftTotalITOG_' + Number(it)).val() != undefined )
        {
            MDtITOG.push(MDT);
        }
    }
...
function ZayavkaTotalEditSave() {
    ...
    for (var it = 1; it <= tList.rows.length; it++) {
        ...
        var MDT =
        {
            'IdNeft': $('#IDneftTotalITOGG_' + Number(it)).val(),
            'Massa': $('#PlanVITOG_' + Number(it)).val(),
            'IdTd': $('#IDT_' + Number(it)).val(),
        }
              
        
        if ($('#IDneftTotalITOGG_' + Number(it)).val() != undefined) {
            MDtITOG.push(MDT);
        }
...
//----------------------------------------------------//
//----------Печать д/з на доставку ------------------------------------------------
function PrintZayTotalWordDost() {

    var stringhref = "/Home/PrintZayTotalWordDost?";
...
function PrintLoginPassword() {
        
    var stringhref = "/Home/PrintLoginPassword?";
...
function VostOstat(ID) {
    $.ajax({
        url: "/Home/VostOstat/" + ID,
        type: "GET",
        contentType: "application/json;charset=UTF-8",
        data: JSON.stringify(),
        dataType: "html",
...
//-------Восстановление остатка удаленного----------//
...
//-----------------------------//
//Восстановление удаленных остатков//
function VostOstatOK(ID) {


    $.ajax({
        url: "/Home/VostOstatOK/" + ID,
        type: "GET",
        contentType: "application/json;charset=UTF-8",
        data: JSON.stringify(),
        dataType: "html",
...
function VostZayOK(ID) {


    $.ajax({
        url: "/Home/VostZayOK/" + ID,
        type: "GET",
        contentType: "application/json;charset=UTF-8",
        data: JSON.stringify(),
        dataType: "html",
